...
public class DataAcquisition implements TimerListener {
	...
	private static final int FWDSPEED = 15;
	public static final int TIMEOUT=40000;
	public static final int SLEEPINT=500;
	...
	// Get an instance of a sample provider for each sensor. Operating // modes are specified in the constructor. Note that the color
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// Enable exception handler myTimer.start();
		myTimer.start();
		 // The Main thread continually updates the display // and checks for stop
		while(rolling) {
			status = Button.readButtons(); 
			if((status == Button.ID_ENTER) || (numSamples >= NSAMPLES)) {
				System.exit(0);
			}
			t.drawInt(numSamples, 4, 11, 4);
			t.drawInt(currentSample,4,11,5);
